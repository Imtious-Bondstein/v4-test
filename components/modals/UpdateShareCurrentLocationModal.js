...
function UpdateShareCurrentLocationModal({
  selectedVehicles,
  shareCurrentLocationModalIsOpen,
  setShareCurrentLocationModalIsOpen,
  isBack,
  setIsBack,
  selectedDuration,
  setSelectedDuration,
  ...
  function closeModal() {
    setShareCurrentLocationModalIsOpen(false);
  }
  ...
  useEffect(() => {
    if (selectedVehicles.length === 1) {
      ...
      setSelectedVehicle({
        value: selectedVehicles[0].identifier,
        label: selectedVehicles[0].bst_id + " " + selectedVehicles[0].vrn,
      });
    } else {
  ...
  return (
    <div>
      <Modal
        isOpen={shareCurrentLocationModalIsOpen}
        ariaHideApp={false}
      ...
      >
        ...
        <h1 className="text-base sm:text-2xl font-bold text-center mb-2">
          Update Share Location
        </h1>
        {/* TOOGLE BUTTON */}
        <div className="flex items-center justify-center space-x-3">
          <p className="text-sm md:text-base">Link Share</p>
        ...
        <div>
          {!isBack ? (
            <div>
              ...
              <div className="mt-4">
                {/* BSTID */}
                {selectedVehicles.map(({ bst_id, activeStatus, id }, index) => {
                  return (
                    <div key={index}>
                      <p className="text-center font-bold text-sm pb-4">
                        {bst_id}
                      </p>
                    </div>
                  );
              ...
              <div className="mt-6">
                ...
                >
                  {isLoading ? "Loading..." : "Update Sharing"}
                </button>
              </div>
...
export default UpdateShareCurrentLocationModal;
