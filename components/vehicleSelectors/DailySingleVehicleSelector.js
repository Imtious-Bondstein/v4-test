...
const DailySingleVehicleSelector = ({
  isRequesting,
  selectedDate,
  setSelectedDate,
  ...
  const handleSingleSelectVehicle = (vehicle) => {
    ...
    if (!selectedDate) {
      ...
      if (
        ...
        //     dateRange.endDate
        // );
        setSelectedVehicleIdentifier(vehicle.v_identifier);
      }
    }
  };
  ...
  const handleSelectMonth = (date) => {
    //parse date and change month only and set it to setSelectDate
  ...
  const handleSelectYear = (date) => {
    //parse date and change year only and set it to selectDate
    const newDate = new Date(selectedDate);
  ...
  return (
    <div className="relative lg:top-32 lg:-mt-[18px] w-[327px] h-[78.5vh] rounded-xl bg-white p-4 overflow-hidden">
      <div className="h-[200px] absolute top-5">
        ...
        <div className="w-full relative tmv-shadow rounded-xl mt-2 mb-4">
          <div className="h-[45px] flex items-center">
            <DatePicker
              selected={selectedDate}
      ...
      {isLoading ? (
        <div className={`h-[350px] mt-[190px] overflow-hidden`}>
        ...
        <div className="calc-height-200 mt-[190px] overflow-y-scroll pr-3.5 select-none scrollGray">
...
export default DailySingleVehicleSelector;
