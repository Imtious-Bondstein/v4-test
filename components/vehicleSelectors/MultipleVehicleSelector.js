...
const MultipleVehicleSelector = ({
  ...
  const [allVehiclesLength, setAllVehiclesLength] = useState(0);
  ...
  const filterSelectedVehicles = async (updatedData) => {
    ...
    getMultipleSelectedVehicles(selectedVehicles, vehicleLists);
  };
  ...
  useEffect(() => {
    const currentStatus = router.asPath.split("status=")[1];
    if (!isLoading) {
      console.log("call use effect");
      if (isSelected) {
        initVehicles(vehicleLists, true); // check
        setIsSelectedAll(true);
        !currentStatus && handleMultipleSelectVehicles();
      } else {
  ...
  useEffect(() => {
    const currentStatus = router.asPath.split("status=")[1];
    if (currentStatus) {
      !isRequesting && handleMultipleSelectVehiclesFilter();
    } else {
      !isFirstCall.current &&
        !isRequesting &&
  ...
  return (
    <div className="relative top-5 lg:top-16">
      <div className="multiple-vehicle-selector-sidebar relative w-[327px] h-[74.3vh] rounded-xl bg-white p-4 overflow-hidden">
                ...
                <div key={index}>
                  ...
                  {group.vehicles.map((vehicle, index) => (
                    <div
                      ...
                      className="flex justify-between  mb-2 rounded-xl p-4 cursor-pointer"
                      key={index}
                    >
...
export default MultipleVehicleSelector;
