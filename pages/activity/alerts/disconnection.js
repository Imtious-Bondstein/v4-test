...
import DisconnectionAlertTable from "@/components/tables/DisconnectionAlertTable";
...
const disconnection = () => {
  ...
  //   axios
  //     .post("/api/v4/alert-management/disconnection-alert", data)
  //     .then((res) => {
  //       const vehicleList = res.data.disconnectionAlert.data;
  ...
  const searchMultipleVehicles = (id, pageNumber) => {
    ...
    axios
      .post("/api/v4/alert-management/disconnection-alert", data)
      .then((res) => {
        const vehicle_list = res.data.disconnectionAlert.data;
        const current_page = res.data.disconnectionAlert.current_page;
        const per_page = res.data.disconnectionAlert.per_page;

        if (res.data.disconnectionAlert.total !== totalItems) {
          setTotalItems(res.data.disconnectionAlert.total);
        }
        ...
        console.log("-- multi res------", res.data.disconnectionAlert);
      })
      .catch((err) => {
        console.log(err);
      })
  ...
  const searchSingleVehicle = async (id, pageNumber) => {
    ...
    await axios
      ...
      .then((res) => {
        ...
        console.log("-- single res--", res.data.disconnectionAlert);
      })
      .catch((err) => {
        console.log(err);
      })
...
disconnection.getLayout = function getLayout(pages) {
  return (
    <ProtectedRoute>
      <Layout>
