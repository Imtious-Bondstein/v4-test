...
import FatigueAlertTable from "@/components/tables/FatigueAlertTable";
...
const driving_fatigue = () => {
  ...
  const searchMultipleVehicles = (id, pageNumber) => {
    ...
    console.log(data);
    axios
      .post("/api/v4/alert-management/fatigue-alert", data)
      .then((res) => {
        const vehicle_list = res.data.fatigueAlert.data;
        const current_page = res.data.fatigueAlert.current_page;
        const per_page = res.data.fatigueAlert.per_page;

        if (res.data.fatigueAlert.total !== totalItems) {
          setTotalItems(res.data.fatigueAlert.total);
        }
        ...
        console.log("-- multi res------", res.data.fatigueAlert);
      })
      .catch((err) => {
        console.log(err);
      })
  ...
  const searchSingleVehicle = async (id, pageNumber) => {
    ...
    await axios
      ...
      .then((res) => {
        ...
        console.log("-- single res--", res.data.fatigueAlert);
      })
      .catch((err) => {
        console.log(err);
      })
...
export default driving_fatigue;

driving_fatigue.getLayout = function getLayout(pages) {
  return (
    <ProtectedRoute>
      <Layout>
