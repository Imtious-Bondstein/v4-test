...
import PanicAlertTable from "@/components/tables/PanicAlertTable";
...
const panic = () => {
  ...
  //   axios
  //     .post("/api/v4/alert-management/panic-alert", data)
  //     .then((res) => {
  //       const vehicleList = res.data.panicAlert.data;
  ...
  const searchMultipleVehicles = (id, pageNumber) => {
    ...
    axios
      .post("/api/v4/alert-management/panic-alert", data)
      .then((res) => {
        const vehicle_list = res.data.panicAlert.data;
        const current_page = res.data.panicAlert.current_page;
        const per_page = res.data.panicAlert.per_page;

        if (res.data.panicAlert.total !== totalItems) {
          setTotalItems(res.data.panicAlert.total);
        }
        ...
        console.log("-- multi res------", res.data.panicAlert.data);
      })
      .catch((err) => {
        console.log(err);
      })
  ...
  const searchSingleVehicle = async (id, pageNumber) => {
    ...
    await axios
      ...
      .then((res) => {
        ...
        console.log("-- single res--", res.data.panicAlert);
      })
      .catch((err) => {
        console.log(err);
      })
  ...
  const visiblePages = 2; //visible pagination buttons
...
panic.getLayout = function getLayout(pages) {
  return (
    <ProtectedRoute>
      <Layout>
