...
import RouteShareListTable from "@/components/tables/RouteShareListTable";
...
const routeShareList = () => {
  const [isLoading, setIsLoading] = useState(false);
  const [initialData, setInitialData] = useState([]);
  const [routeShareData, setRouteShareData] = useState([]);
  ...
  // ROUTE SHARE DATA INIT FUNCTION (HANDLE SL NUMBER) ======================
  const initRouteShareData = (notification, current_page, per_page) => {
    const updatedData = notification.map((items, index) => ({
      ...items,
      checkbox: false,
      sl: per_page * (current_page - 1) + index + 1,
    }));
    setRouteShareData(updatedData);
  };
  ...
  const handleSearch = async (pageNumber) => {
    ...
    await axios
      ...
      .then((res) => {
        ...
        initRouteShareData(res.data.data.data, current_page, per_page);

        console.log("API Response", res.data.data.data);
      })
      .catch((err) => {
        setRouteShareData(routeShareData);
        setIsLoading(false);
        console.log(err);
  ...
  const fetchRouteShareList = async (pageNumber) => {
    setIsLoading(true);
    await axios
      .get(
        `/api/v4/shared-token/list?token_type=vehicleRoute&offset=${offset}&page=${
  ...
  const handlePageClick = (page) => {
    setCurrentPage(page);
    searchKey.length === 0 ? fetchRouteShareList(page) : handleSearch(page);
  };
  ...
  useEffect(() => {
    ...
    searchKey.length === 0 ? fetchRouteShareList(1) : handleSearch(1);
  }, [offset]);
  ...
  useEffect(() => {
    ...
    if (searchKey && searchKey.length > 0) {
      ...
      setIsLoading(false);
      setRouteShareData(initialData);
      initRouteShareData(initialData, 1, offset);
...
export default routeShareList;

routeShareList.getLayout = function getLayout(page) {
  return (
    <ProtectedRoute>
      <Layout>
